                ...
                using (var streamReader = new StreamReader(contentStream)) {
                    ...
                    try {
                        using(var tr = new StringReader(content)) {
                            return (XmlRpcResponse) Deserializer.Deserialize(tr);
                        }
                    } catch(Exception ex) {
                        string fname = string.Join("-", System.Diagnostics.Process.GetCurrentProcess().Id.ToString(),
                            "content");
                        System.IO.File.WriteAllText($"../../slbots_logs/http/xml-login-errors/{fname}", content);
